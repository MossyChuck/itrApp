    ...
    $scope.addNewUser = function () {
        $http.post('/registerUser', $scope.newUser).then(function (responce) {
            ...
            if (responce.data === 'send') {
                $scope.$emit('changeContentUrl', { url: '/htmls/content/en/message.en.html', message: 'Verification email is send to you.' });
            }
            if (responce.data === 'verifyed') {
                $scope.$emit('changeContentUrl', { url: '/htmls/content/en/message.en.html', message: 'Email is verifyed' });
            }
        });
    };
});
